...
namespace CODE.Framework.Utilities.Mapper
{
    /// <summary>
    ...
    public class Mapper
    {
        ...
        protected virtual void MapEnumValues(object source, object destination, PropertyInfo property, PropertyInfo sourceProperty, MappingOptions options)
        {
            ...
            {
                enumCounter++;
                if (string.Compare(newEnumValue, currentName, options.MatchEnumCase) != 0) continue;
        ...
        protected virtual string GetMappedSourcePropertyName(string destinationProperty, MapDirection direction, string destinationContainer, string sourceContainer)
        {
            foreach (var map in _maps)
                if (string.Compare(sourceContainer, map.SourceContainer, false) == 0 && string.Compare(destinationContainer, map.DestinationContainer) == 0)
                    if (direction == MapDirection.Forward)
                    {
                        if (string.Compare(destinationProperty, map.DestinationField, false) == 0)
                        {
                    ...
                    {
                        if (string.Compare(destinationProperty, map.SourceField, false) == 0)
                        {
